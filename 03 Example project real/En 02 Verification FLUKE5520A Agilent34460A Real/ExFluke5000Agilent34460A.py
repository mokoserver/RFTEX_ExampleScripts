...
############################################   Agilent34460A SET ACBand   #############################################
            if self.Simulation:
                ...
                RFSE.Driver('AgilentDMM', 'set', f'range = {range}')
                RFSE.Driver('AgilentDMM', 'set', f'ACBand = {filter}')
            else:
                RFSE.Messenger("set", "Make settings on Agilent34460A#@agilent34460a",
                               f"Make settings:\nSet range = {range}\nSet ACBand = {filter}\nPress OK")
...
#############################################   Fluke5520A SET R2   ###################################################
            if self.Simulation:
                ...
                RFSE.Driver('Fluke5000', 'set', f'R2 = {verified}')
            else:
                RFSE.Messenger("set", "Make settings on Fluke5520A#@fluke5520a",
                               f"Make settings:\nSet R2 = {verified}\nPress OK")
#######################################################################################################################
...
#############################################   Fluke5520A SET R4  ###################################################
            ...
            if self.Simulation:
                ...
                RFSE.Driver('Fluke5000', 'set', f'R4 = {verified}')
            else:
                RFSE.Messenger("set", "Make settings on Fluke5520A#@fluke5520a",
                               f"Make settings:\nSet R4 = {verified}\nPress OK")
#######################################################################################################################
...
##########################################   AGILENT34460A SET Reset  #################################################
        ...
        if type_setting_agilent == 'Automatic':
            ...
            agilent3401a_status = RFSE.Driver('AgilentDMM', 'init', '')
        ...
        if type_setting_agilent == 'Manual':
            ...
            RFSE.Report("TYPE_SETTING_AGILENT34460A", "set", 'string', 'Simulation')
...
################################################   Fluke5520 SET Reset  ###############################################
        ...
        if type_setting_fluke == 'Automatic':
            ...
            fluke_5520a_status = RFSE.Driver('Fluke5000', 'init', '')
